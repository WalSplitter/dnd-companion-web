...
const KIND_LABEL: Record<string, string> = { melee: 'Melee', ranged: 'Ranged', thrown: 'Thrown' }
        ...
        {attacks.map((attack) => (
          <li key={attack.name} className="rounded-lg border border-border bg-surface-2 p-2.5">
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
              ...
              <span className="text-xs uppercase text-fg-muted">{KIND_LABEL[attack.kind]}</span>
              ...
              <span className="ml-auto flex items-center gap-1.5">
                <D20RollButton label={`${attack.name} attack`} modifier={attack.attack_bonus} />
